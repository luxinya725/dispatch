...
# override=True：.env 优先于外部环境变量。
# 部分终端会全局预置 ANTHROPIC_BASE_URL 等变量，不覆盖会导致读到错误的模型服务地址。
...
# 按 IP 限流：公开部署时防止 LLM 额度被脚本刷爆。
# 通过 CHAT_RATE_LIMIT 调整，设为空字符串可关闭。
_CHAT_RATE_LIMIT = os.getenv("CHAT_RATE_LIMIT", "20/hour")
...
# ── 前端静态资源 ──────────────────────────────────────────────────────────────
# 单体部署：前端构建产物存在时由本服务一起托管，访问根路径即打开页面。
# 必须放在所有 API 路由之后注册，否则会抢占 /chat 等路径。
_FRONTEND_DIR = pathlib.Path(os.getenv("FRONTEND_DIST", pathlib.Path(_ROOT) / "static"))
if _FRONTEND_DIR.is_dir():
    app.mount("/", StaticFiles(directory=str(_FRONTEND_DIR), html=True), name="frontend")
    logger.info(f"前端静态资源已挂载: {_FRONTEND_DIR}")
